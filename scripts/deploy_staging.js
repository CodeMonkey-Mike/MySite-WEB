...
const PORT = 9028;
const SERVED_FOLDER = '/home/dominitech/workspace/mikeneder.me/mikeneder-web/master';
const DOMAIN = 'mikeneder.me';
...
const main = async () => {
  try {
    ...
    await exec(`echo '${SUDO_PASSWORD}' | sudo -S cp -r ${SERVED_FOLDER}/public /var/www/${SITE_URL}`);
    // Reload staging site
    await exec('/home/dominitech/.npm-global/bin/pm2 reload mn-staging');
    ...
    const cleanup = async (prNumber) => {
      ...
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm -rf /var/www/qa.${DOMAIN}/qa${prNumber}.${DOMAIN}`);
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm -rf /home/dominitech/workspace/${DOMAIN}/mikeneder-web/${prNumber}`);
      await exec(`/home/dominitech/.npm-global/bin/pm2 delete qa${prNumber}.${DOMAIN}`);
      await exec('/home/dominitech/.npm-global/bin/pm2 save');
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm /etc/nginx/sites-available/qa${prNumber}.${DOMAIN}`);
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm /etc/nginx/sites-enabled/qa${prNumber}.${DOMAIN}`);
